...
### V0.0.8 Beta V1.0.8 ###
### Author: Janosch Klawatsch, 08/09.07.2026 ###
### game file V0.0.0 ###
...
from persistence import delete_match, list_history, load_match, save_match # For the save and history parts

### -Classes- ###
### Game ###
class Game: # Create a class
    ### Classic "GAME" Functions ###
    def __init__(self): # Every class need a init with the start parameters
        ...
        # Hints & Undo
        self.hints_left = config.HINTS_PER_GAME # Sets the number of hints of a round
        ...
        # History
        self.history_files = []           # Remembers all played games
        ...
        self.selected_history_name = None # And the name of them
    ...
    def new_game(self, n): # Creates a new game depending on the size given
        ...
        self.start_time = pygame.time.get_ticks() # Start time is now
        self.play_time = 0 # But not played yet
        self.hints_left = config.HINTS_PER_GAME # reset the number of hints available
    ...
    def tick_timer(self): # Counts the time
        if self.state == "PLAY" and not self.won and not self.paused: # If the player is playing
            self.play_time = pygame.time.get_ticks() - self.start_time - self.total_paused_ms # Time we have without the time we started and the time we haven't played
    ...
    def toggle_pause(self): # Collect the pause
        ...
        if self.paused: # If we are in pause
            self.pause_started_at = pygame.time.get_ticks() # Pause started now
        else:
            self.total_paused_ms += pygame.time.get_ticks() - self.pause_started_at # Total time is the time it already is and also the time it was paused until.
    ...
    def use_hint(self): # To handle the usage of a hint
        ...
        self.hint_flash_timer = pygame.time.get_ticks() # Get the time
        self.current_game_actions.append({
            "time": self.play_time,
            "type": "Hint",
            "r": r, "c": c,
    ...
    def _refresh_win_state(self): # Handle the win check
        ...
        if self.won: # If the player actually won
            if self.save_history: # If the user wants that his data get saved
                hints_used = config.HINTS_PER_GAME - self.hints_left # Number of used hints
    ...
    def open_history(self): # Open the history view
        self.history_files = list_history() # Need the files to show
    ...
    def delete_history(self, filename): # Delete a given file from the storage
        ...
        self.history_files = list_history() # Update the enable history