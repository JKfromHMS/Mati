...
def main(): # The main function
    pygame.init() # Pygame need to be initialized
    screen = pygame.display.set_mode((config.WIDTH, config.HEIGHT)) # Define the size of the window
    pygame.display.set_caption("Mati") # The shown name of the window
    
    # Because fonts need pygame, they are defined in this file and not in config. But maybe they could be in widgets.
    title_font = pygame.font.SysFont("arial", 48, bold=True) # For the biggest texts
    font = pygame.font.SysFont("arial", 28, bold=True) # For the normal texts and buttons
    small_font = pygame.font.SysFont("arial", 22) # For small  texts and most buttons
    ...
    while True: # Until the programm gots quit
        screen.fill(config.BG_COLOR) # Draw the background
        mx, my = pygame.mouse.get_pos() # Get the mouse position
        
        for event in pygame.event.get(): # Everything happend got saved
            if event.type == pygame.QUIT: # If the user want to quit
                pygame.quit() # End the game engine
                sys.exit() # Close the game
            
            elif event.type == pygame.KEYDOWN: # If the player pressed a key
                if event.key == pygame.K_p and game.state == "PLAY": # If the player pressed p in the game
                    game.toggle_pause() # Make the game paused or continued
                elif event.key == pygame.K_z and (event.mod & pygame.KMOD_CTRL) and game.state == "PLAY": # If the player pressed CTRL and z and is playing
                    game.undo() # Make the last move undone
                    
            elif event.type == pygame.MOUSEBUTTONDOWN: # If the player clicked the mousebutton
                handle_click(game, event, mx, my) # Check what for a click
            
            elif event.type == pygame.MOUSEWHEEL and game.state == "HISTORY": # If the player tries to scroll in history view
...
def handle_click(game, event, mx, my): # Let the programm work with a click
    ...
    if state == "MENU": # If the player is in the menu
        ...
        for n in config.DIFFICULTIES: # For every grid size
        ...
        if offset_x + config.CELL_SIZE <= mx < offset_x + (game.n + 1) * config.CELL_SIZE and \
           offset_y + config.CELL_SIZE <= my < offset_y + (game.n + 1) * config.CELL_SIZE: # If a cell got a click
            c = (mx - offset_x - config.CELL_SIZE) // config.CELL_SIZE # The column is
            r = (my - offset_y - config.CELL_SIZE) // config.CELL_SIZE # The row is
...
def handle_history_scroll(game, event): # Make scrolling possible
    ...
    min_scroll = -max(0, (len(game.history_files) * 60) - (config.HEIGHT - 150)) # Not under the bottom
