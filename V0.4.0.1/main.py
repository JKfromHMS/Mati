...
### Author: Janosch Klawatsch, 08/09/10.08.2026 ###
### main file V0.4.2 ###
    ...
    while True: # Until the programm gots quit
        ...
        for event in pg.event.get(): # Everything happend got saved
            if event.type == pg.QUIT: # If the user want to quit
                ...
                if event.key == pg.K_F11: # If f11 pressed
                ...
                elif (event.key == pg.K_m) and (event.mod & (pg.KMOD_CTRL | pg.KMOD_META)) and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE", "HANNAH", "PLAY", "TERMINAL", "STATS"):
                    if game.state == "PLAY":
                ...
                elif game.alt_control and event.key in (pg.K_SPACE, pg.K_RETURN) and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE", "HANNAH", "STATS"): # Keyboard click replacement
                    ...
                    if active_terminal.should_close: # If the player want to close or switch
                        ...
                        game.refresh_progress() # Refresh the found terminal and 42 easter egg
                    ...
                    game.mark_terminal_visited() # Remember the player found the terminal
        ...
        if game.state != "TERMINAL" and active_terminal:
            ...
            game.refresh_progress() # Make sure the visit, if it was the first, is saved
...
def _scrollbar_target(game): # Which scrollbar (if any) is relevant
    if game.state == "HISTORY": # The history list
        ...
        visible = con.HEIGHT - (con.LIST_TOP - 10) # The visible height
    ...
    if game.state == "HANNAH" and getattr(game, "hannah_open_index", None) is None: # The tile view
        ...
        return ("hannah", track, s.hannah_content_width(), con.WIDTH - 80, False) # Horizontal bar
...
def draw(game, mx, my): # Let the programm draw the views
    ...
    if game.state not in ("ABOUT", "HANNAH", "HISTORY_DETAIL"): 
