...
### Author: Janosch Klawatsch, 10/11.08.2026 ###
### game file V0.4.2 ###
...
import os # For the export folder
...
class Game: # Create a class
    ...
    def __init__(self, sounds): # Every class need a init with the start parameters
        ...
        self.settings, self.stats, self.paused_games, self.hannah_solved, self.progress = load_settings_and_stats() # Load the settings, statistics and paused games
        self.apply_settings()
    ...
    def mark_terminal_visited(self): # Remember that he player know the terminal
        if not self.progress.get("visited_terminal"): # Only write once
            self.progress["visited_terminal"] = True # Remember it
            save_settings_and_stats(self.settings, self.stats, self.paused_games, progress=self.progress) # Save the change
            
    def mark_found_42(self): # Remember if the player found the 42 easter-egg
        if not self.progress.get("found_42"): # Only write once
            self.progress["found_42"] = True # Remember it
            save_settings_and_stats(self.settings, self.stats, self.paused_games, progress=self.progress) # Sace the change
            
    def refresh_progress(self): # reload the badges
        _, _, _, _, self.progress = load_settings_and_stats() # Load the progress
    ...
    def export_selected_to_mp4(self): # Turn the currently game into a mp4 file
        ...
        folder = self._ask_export_folder() # Make sure the export folder is there
        if not folder:
            return
        
        base = (self.selected_history_name or dt.now().strftime("%Y-%m-%d_%H-%M-%S")).replace("_", " ").split(".")[0] # Get the base name of the game
        out_path = os.path.join(con.EXPORT_DIR, base + ".mp4") # Where we will save the video
    ...
    def _ask_export_folder(self): # Opens the folder picker
        try:
            ...
            folder = filedialog.askdirectory(title="Choose a folder to save the mp4 in", initialdir=initial) # The actual picker
            root.destroy() # Clean up after picking
            if folder:
                self.last_export_folder = folder # Remember for next time
            return folder or None
        except Exception: # Any failure (expected if tkinter is not running fine)
            fallback = os.path.join(os.getcwd(), con.EXPORT_DIR) # The none choosen location
            os.makedirs(fallback, exist_ok=True) # Make sure it exist
            return fallback # use this instead
    ...
    def _hannah_scroll_to_next(self, from_index): # Centers the solved tile
        ...
        natural_rect = s.hannah_tile_rect(next_index, 0) # Where this tile already is
        target_scroll = con.WIDTH // 2 - natural_rect.centerx # How far to scroll
        total_width = s.hannah_content_width() # The whole strip's width
        min_scroll = -max(0, total_width - (con.WIDTH - 80)) # Scroll right
        self.hannah_scroll_x = max(min_scroll, min(0, target_scroll)) # Scroll right other direction
