...
### Author: Janosch Klawatsch, 08-11.08.2026 ###
### screens file V0.4.3 ###
...
def _hardcoded_neighbor(game, key, direction):
    ...
    match state:
        ...
        case "SETTINGS":
            terminal_found = bool(getattr(game, "progress", {}).get("visited_terminal"))
            if terminal_found:
                neighbors = {
                    ...
                    "stats": {"down": None, "up": f'{"toggle_ms" if game.timer_enabled else "toggle_timer"}', "left": None, "right": "about"},
                    "about": {"down": None, "up": "toggle_live_clock", "left": "stats", "right": None},
                }
            else:
                neighbors = {
                    "back": {"down": "toggle_history", "up": None, "left": None, "right": "toggle_history"},
...
_OPPOSITE_DIRECTION = {"up": "down", "left": "right"}
...
def _settings_back_neighbor(game, direction): # To follow the keyboard hover to send the player better back.
    ...
    if remembered:
        ...
        if direction == _OPPOSITE_DIRECTION.get(arrival_direction):
...
def move_focus(game, direction): # How to move the focus
    ...
    if state == "SETTINGS":
        ...
        if key == "toggle_alt_control":
            ...
            if nxt == "toggle_alt_control" and key != "toogle_alt_control":
...
def _draw_menu_badges(game): # Show if something was found
    terminal_found = bool(getattr(game, "progress", {}).get("visited_terminal")) # Terminal already opened
    found_42 = bool(getattr(game, "progress", {}).get("found_42")) # Already tipped 42 in ultra game
...
def _settings_layout(game): # computes button rects and headers
    ...
    bottom_y = 525
    buttons["stats"] = pg.Rect(con.WIDTH // 2 - 165, bottom_y, 160, 34) # Stats button
    buttons["about"] = pg.Rect(con.WIDTH // 2 + 5, bottom_y, 160, 34) # About button
...
def draw_settings(game, mx ,my): # Draw the defined settings
    ...
    for title, x, y in headers: # Every section gets a small label above
        w.draw_tiny(title.upper(), con.DIMMED_TEXT_COLOR, x, y, center=False)
...
def draw_stats(game, mx, my): # Draws the graphical version of the stats
    ...
    col_x = [80, 260, 420, 560, 700] # Column position
    ...
    for n in con.DIFFICULTIES: # Every grid size
        for mode_label, mode_key in (("Normal", "normal"), ("Ultra", "ultra")): # both modes
...
def history_scrollbar_track():
    return pg.Rect(con.ENTRY_X + con.ENTRY_WIDTH + 6, con.LIST_TOP - 10, con.SCROLLBAR_WIDTH, con.HEIGHT - (con.LIST_TOP - 10))
...
def detail_scrollbar_track():
    return pg.Rect(790 - con.SCROLLBAR_WIDTH - 2, 120, con.SCROLLBAR_WIDTH, 438)
...
def draw_history_detail(game, mx, my): # the draw the detailed history view
    ...
    if n != 7:
        ...
        if is_start_view: # The start of the game
            w.draw_tiny("Action: Start", con.TEXT_COLOR, offset_x, footer_y + 40, center=False) # Describe it as start
            ...
            w.draw_tiny(f"Action: Row: {last_action['r'] + 1} Column: {last_action['c'] + 1} - {label}", con.TEXT_COLOR, offset_x, footer_y + 40, center=False)
        else: 
    ...
    track = pg.Rect(540 + 250 - con.SCROLLBAR_WIDTH - 2, 120, con.SCROLLBAR_WIDTH, 438)
