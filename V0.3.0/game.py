...
class Game: # Create a class
    ...
    def __init__(self, sounds): # Every class need a init with the start parameters
        ...
        self.sounds.enabled = True # Default until settings changed
        ...
        self.settings, self.stats, self.paused_games = load_settings_and_stats() # Load the settings, statistics and paused games
    ...
    def move_cursor(self, key): # Move the keyboard cursor
        ...
        elif key == pg.K_DOWN: self.cursor_r = min(self.n - 1, self.cursor_r + 1) # Move down the grid
        elif key == pg.K_LEFT: self.cursor_c = max(0, self.cursor_c - 1) # Move left the grid
        elif key == pg.K_RIGHT: self.cursor_c = min(self.n - 1, self.cursor_c + 1) # Move right the grid
