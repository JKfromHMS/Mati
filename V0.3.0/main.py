...
### V0.3.0 Beta V1.0.14 ###
### Author: Janosch Klawatsch, 20.07.2026 ###
### main file V0.3.1 ###
        ...
        for event in pg.event.get(): # Everything happend got saved
            if event.type == pg.QUIT: # If the user want to quit
                ...
                if event.key == pg.K_F11: # If f11 pressed
                    ...
                    elif event.key == pg.K_l: game.click_cell(game.cursor_r, game.cursor_c) # Make a left click
                ...
                elif game.alt_control and event.key in (pg.K_UP, pg.K_DOWN, pg.K_LEFT, pg.K_RIGHT) and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE"): # Keyboard menu navigation
                ...
                elif game.alt_control and event.key == pg.K_SPACE and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE"): # Keyboard click replacement
                    order = s.focus_order(game) # Get the navigable elements of this screen
                    ...
                    if active_terminal.should_close: # If the player want to close
                        game.state = "MENU" # Got to the Menu
...
def handle_click(game, click_time, mx, my, button): # Let the programm work with a click
    ...
    if state == "MENU": # If the player is in the menu
    ...
    elif state == "DELETE_HISTORY": # The delete screen
    ...
    elif state == "RESUME_CHOICE": # If the player is asked if he want to continue
        ...
        if buttons["resume"].collidepoint(mx, my): # If the player want to resume
            ...
            return # task is done
        elif button["new"].collidepoint(mx, my): # If the player ant create a new
            game.discard_and_start(game.pending_new_n, game.pending_new_ultra) # throw the paused away
            return # task is done
        elif buttons["cancle"].collidepoint(mx, my): # If the player want to cancle
            game.state = "MENU" # Set state back to menu
            return # task is done
                
    elif state == "HANNAH": # If the player is in the first easter egg
...
def draw(game, mx, my): # Let the programm draw the views
    ...
    elif game.state == "RESUME_CHOICE": # If deciding about continuing the match or starting a new
    ...
    elif game.state == "PLAY": # In a game
        s.draw_play(game, mx, my) # Creates the game view
    elif game.state == "DELETE_HISTORY": # If in delete history screen
        s.draw_delete_confirm(game, mx, my) # Creates the delete screen
    elif game.state == "HANNAH": # If the player is in the first easter egg
