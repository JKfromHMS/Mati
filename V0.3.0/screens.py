...
def ensure_focus_visible(game, order): # Makes sure in lists the focused is visible
    if not order: # Nothing to check
        return # Do nothing
    idx = game.focus_index % len(order) # Get the index of the buttons
    key, rect = order[idx] # Load the currently focused element
...
def draw_menu(game, mx, my): # Draw the defined menu
    ...
    fk = _focused_key(game) # Get the current keyboard cursor position
...
def draw_settings(game, mx ,my): # Draw the defined settings
    ...
    fk = _focused_key(game) # Get the current keyboard focus
...
def draw_about(game, mx, my):
    ...
    fk = _focused_key(game) # Get the keyboard focus
...
def draw_delete_confirm(game, mx, my): # Drawn the menu to delete a file
    ...
    fk = _focused_key(game) # Get the current keyboard
...
def hannah_buttons(): # Defines the buttons for the easter egg
    offset_x = offset_y = 32.5 # The get away from the left top corner
