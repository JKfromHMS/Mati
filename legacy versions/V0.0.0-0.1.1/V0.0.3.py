...
GREEN = (50, 180, 50) # -New- Added a green color
CELL_SIZE = 60
...
SMALL_FONT = pygame.font.SysFont("arial", 24) # Added a small font shortcut
...
### Win Check ###
def check_win(grid, user_sel, row_sums, col_sums, n): # Added a complete new function system
    # Check the rows
    for r in range(n):
        ...
        if state == "MENU":
            ...
            for r in range(n):
                for c in range(n):
                    ...
                    draw_text(str(grid[r][c]), FONT, TEXT_COLOR, SCREEN, rect.centerx, rect.centery) 
                    
                    draw_outer_border(SCREEN, offset_x, offset_y, n, CELL_SIZE) # -New- Give the grid a border
                    draw_fulfilled_indicators(SCREEN, grid, user_sel, row_sums, col_sums, n, offset_x, offset_y, CELL_SIZE) # -New- Give the sums a mark if they are reached
