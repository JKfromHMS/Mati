...
DIMMED_TEXT_COLOR = (180, 180, 180)  # -New- Added a color for dimmed numbers
CELL_SIZE = 60
...
def main():
    ...
    user_dimmed = [] # -New- Another 2D Array for the dimmed state.
    won = False
    
    # Add buttons for the screens
    btn_start_4 = pygame.Rect(WIDTH//2 - 100, 200, 200, 50) # Different Buttons for different Sizes
    btn_start_5 = pygame.Rect(WIDTH//2 - 100, 270, 200, 50) # Different Buttons for different Sizes
    btn_start_6 = pygame.Rect(WIDTH//2 - 100, 340, 200, 50) # Different Buttons for different Sizes
    ...
    while True:
        ...
        row_fulfilled = [False] * (n+1)
        col_fulfilled = [False] * (n+1)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if state == "MENU":
                    ...
                    if state == "PLAY":
                        ...
                        user_dimmed = [[False] * n for _ in range(n)] # -New- Fill with False
                        won = False
                    ...
                    if btn_back.collidepoint(mx, my): state="MENU" # Backbutton handling
                        ...
                        if offset_x + CELL_SIZE <= mx < offset_x + (n+1) * CELL_SIZE and \
                           offset_y + CELL_SIZE <= my < offset_y + (n+1) * CELL_SIZE:
                            ...
                            user_dimmed[r][c] = False # -New- If a cell get left clicked end rightclick look
                            user_sel[r][c] = not user_sel[r][c]
                            won = check_win(grid, user_sel, row_sums, col_sums, n)
                            
            elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 3: # -New- Handling what happen with rightclicks
                if state == "PLAY":
                    if not won:
                        offset_x = WIDTH // 2 - ((n+1) * CELL_SIZE) // 2
                        offset_y = HEIGHT // 2 - ((n+1) * CELL_SIZE) // 2
                        ...
                        if offset_x + CELL_SIZE <= mx < offset_x + (n+1) * CELL_SIZE and \
                           offset_y + CELL_SIZE <= my < offset_y + (n+1) * CELL_SIZE:
                            c = (mx - offset_x - CELL_SIZE) // CELL_SIZE
                            r = (my - offset_y - CELL_SIZE) // CELL_SIZE
                            user_dimmed[r][c] = not user_dimmed[r][c] # -New- Toggle for True/False
                            user_sel[r][c] = False # -New- If dimmed it is not clicked
                            won = check_win(grid, user_sel, row_sums, col_sums, n)
        ...
        if state == "MENU":
            ...
            for r in range(n):
                for c in range(n):
                    ...
                    current_text_color = DIMMED_TEXT_COLOR if is_dimmed else TEXT_COLOR
