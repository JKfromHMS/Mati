...
class Terminal: # The hole terminal
    def __init__(self, sounds): # Every class needs an init
        ...
        self.settings, _, _, _, _ = ps.load_settings_and_stats() # Load the settings
    ...
    def _handle_command(self, command): # Handles the commands
        ...
        match self.terminal_mode:
            ...
            case "history":
                self._handle_history_command(c_cmd, command)
                return
    ...
    def _handle_game_command(self, cmd, command): # Handle commands in game mode
        ...
        if command == "42": # Second little easter egg
            ...
            game.mark_found_42() # Remember the easter egg was found.
            return
    ...
    def _handle_history_command(self, c_cmd, command):
        if c_cmd in ("return", "close"):
            self.terminal_mode = None
            self._clear()
            if c_cmd == "close":
    ...
    def _print_stats(self): # Print the stats
        _, stats, _, _, _ = ps.load_settings_and_stats() # Reload stats
    ...
    def _print_history(self, limit=10): # Print the last played matches
        ...
        if len(entries) > limit:
            self._print(f"... and {len(entries) - limit} more.")
        self._print("")
