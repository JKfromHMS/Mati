...
def _render_text_cached(
    ...
    """Renders text ones and reuses it for every later call with the same font/text/color.
    
    Grid digits and sum labels repeat constantly across cells, states and frames, 
    so chaching avoids redoing the comparatively expensive font rasterization every time.
    ...
    Returns:
        pg.Surface: 
        The rendered (and reused) text surface.
    """
...
def _render_board(
    ...
    """Renders the static part of a video frame off-screen for the for the replay video.
    
    Draws everything for a given game state EXCEPT the elapsed time overlay: row/column
    sum targets, grid values, cell selections, dimming states, action highlighs and completion
    rings. This part is identical for every frame between two player actions, so the caller 
    can render it once per game state and reuse it (see export_history_to_mp4), 
    instead of rebuilding it for every single output frame.

    Args:
        data (Dict): Dictionary containing puzzle data.
        n (int): Grid dimension size (N x N)
        sel (List[List[bool]]): 2D boolen grid where True indicates a selected cell.
        dimmed (List[List[bool]]): 2D boolean grid where True indicated a manually dimmed cell.
        highlight_cell (Optional[Tuple[int, int]]): Tuple of (row, col) specifying the active cell, or None.
        highlight_color (Optional[Tuple[int, int, int]]): RGB tuple fro the action highlight border, or None for default gold.
        time_ms (int): The overall time of the match.
        ...
        action_time_ms (int): The explicit time of the action. Defaults to 0.

    Returns:
        pg.Surface: 
        A Pygame Surface object containing the rendered board state, minus the elapsed time text.
    """
...
def _draw_time_overlay(
    ...
    """Draws only the elapsed-time onto an already rendered board surface.
    
    Kept separate from _render_board on purpose: the board itself only changes when a
    player action happens, while the elapsed time changes on virtually every single
    output frame. Splitting them lets the caller cache and reuse the (expensive) board
    render across every frame that shares the same game state, redoing only this cheap
    text blit every frame.

    Args:
        surf (pg.Surface): The board surface (usually a fresh copy of a cached one) to draw onto.
        time_ms (int): Current playback time in milliseconds.
        tiny_font (pg.font.Font): Small Pygame Font object for the time display.
        text_color (Tuple[int, int, int]): RGB tuple matching the board's normal text color.
        w_px (int): Total frame width in pixels.
        margin (int): The scaled margin to every side.
        cell_size (int): The scaled cell size, used to derive the same header scale as the board.
    """
    ...
    header_scale = cell_size / con.CELL_SIZE 
    _blit_centered(surf, tiny_font, f"Time: {time_str}", text_color, w_px // 2, margin / 2 + 8 * header_scale)
    
    
def _display_actions(data: Dict) -> List[Dict]:
    """Generates the full timeline of game actions, prependend with a synthetic start frame.
    ...
    try:
        ...
        for f in range(total_frames):
            ...
            if state_idx != cached_state_idx:
                act_t, sel, dimmed, hl_cell, hl_color = states[state_idx]
            
                cached_board = _render_board(
                    data, 
                    n, 
                    sel, 
                    dimmed, 
                    ...
                    ultra,
                    quality_scale,
                    action_time_ms = act_t,
                )
                cached_state_idx = state_idx
                
            # Cheap copy of the cached board, then draw only the part that truly changes every frame
            surf = cached_board.copy()
            _draw_time_overlay(surf, display_ms, tiny_font, time_text_color, w_px, margin, cell_size)
