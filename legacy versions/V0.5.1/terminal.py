...
### V0.5.1 Beta V1.0.20 ###
### Author: Janosch Klawatsch, 16-18.08.2026 ###
### terminal file V0.5.3 ###
...
class Terminal: # The hole terminal
    ...
    def _handle_history_command(self, cmd, c_cmd, command):
        ...
        if cmd.startswith("filter "):
            ...
            while i < len(raw_args):
                if i + 1 < len(raw_args) and f"{raw_args[i]}_{raw_args[i + 1]}" in ("hints_up", "hints_down", "size_up", "size_down"):
    ...
    def _help_history(self):
        self._print("filter 'what' - removes all entries not matching with 'what'")
        self._print("sort by 'thing 1-4' - put the wanted to the top in given order")
        self._print("sorted - reset sort to default (newest)")
        self._print("unfiltered - resets all filter")
        self._print("reset - remove all filters and sort to default (newest)")
        self._print("")
        self._print("open real - opens the graphical history menu")
        self._print("return - return to mati terminal")
        self._print("close - leaves the mati terminal to the main menu")
        self._print("")
        self._print("")
        self._print("")
    ...
    def _help_history_filter(self):
        self._print("filter 'what' - removes all entries not matching with 'what'")
        self._print("       4x4 - grid size")
        self._print("       5x5 - grid size")
        self._print("       6x6 - grid size")
        self._print("       7x7 - grid size")
        self._print("       ultra - game mode")
        self._print("       normal - game mode")
        self._print("       no_hints - number hints 0")
        self._print("       one_hint - number hints 1")
        self._print("       two_hints - number hints 2")
        self._print("       three_hints - number hints 3")
        self._print("       under_two_hints - number hints 0, 1")
        self._print("       under_three_hints - number hints 0, 1, 2")
        self._print("       hints_used - number hints 1, 2, 3")
        self._print("       over_one_hint - number hints 2, 3")
        self._print("       under_time 'time in ms' - longer 'time in ms'")
        self._print("       over_time 'time in ms' - shorter 'time in ms'")
        
        
    def _help_history_sort(self):
        self._print("sort by 'thing 1-4' - put the wanted to the top in given order")
        self._print("        oldest - game played")
        self._print("        newest - game played")
        self._print("        fastest - play time")
        self._print("        slowest - play time")
        self._print("        smallest - grid size")
        self._print("        biggest - grid size")
        self._print("        ultra - game mode")
        self._print("        normal - game mode")
        self._print("        hints up - less hints")
        self._print("        hitns down - most hints")
        self._print("")
        self._print(" You can put up too four sorts at ones.")
        self._print(" Like: 'sort by ultra fastest smallest hints up'")
        self._print(" This would bring ultra to the top, between all ultras")
        self._print(" the smallest (4x4) and in those the one with the less hints")
        self._print(" If multiple have same mode size and number of hints, the fastest will be first.")
        self._print(" The four time commands are handled last and can not work at the same time.")
        self._print("")
