    ...
    while True: # Until the programm gots quit
                ...
                if event.button == 1: # Only left clicks
                    if game.state == "ADVANCED_SETTINGS":
                        for which in ("game_volume", "terminal_volume"):
                            track = helpers.advanced_settings_layout(game).get(which)
        ...
        if getattr(game, "slider_drag", None):
            if pg.mouse.get_pressed()[0]:
                track = helpers.advanced_settings_layout(game).get(game.slider_drag)
                if track:
                    value = w.value_for_slider_x(track, mx)
...
def handle_click(game, click_time, mx, my, button): # Let the programm work with a click
    ...
    match state: # py 3.10 method to make if else more efficient
        case "MENU": # If the player is in the menu
            buttons = helpers.menu_buttons() # Load the buttons
        ...
        case "SETTINGS": # If the player is in the settings
            buttons = helpers.settings_buttons(game) # Load buttons
            ...
            #elif buttons["advanced"].collidepoint(mx, my):
            #    game.return_focus["SETTINGS"] = "advanced"
            #    game.language_dropdown_open = False
            #    game.state = "ADVANCED_SETTINGS"
            
        case "ABOUT": # If the player is in the about
            if helpers.BTN_BACK.collidepoint(mx, my): # If the player clicked back
                game.state = "SETTINGS" # Return to settings
                
        case "STATS": # If the player is in the stats
            if helpers.BTN_BACK.collidepoint(mx, my): # If the player clicked on back
                game.state = "SETTINGS" # Return to settings
                
        case "ACHIEVEMENTS":
            if helpers.BTN_BACK.collidepoint(mx, my):
                game.state = "SETTINGS" # Return to settings
            else:
                total_pages = len(con.ACHIEVEMENT_PAGES)
                ...
                if game.achievements_page == 0:
                    ...
                    if games_tile.collidepoint(mx, my):
                        games_prev = helpers.ACH_GAMES_PREV_RECT
                        games_next = helpers.ACH_GAMES_NEXT_RECT
                        games_count = len(con.GENERAL_GAME_MILESTONES)
                        index = getattr(game, "ach_games_index", 0)
                        if index > 0 and games_prev.collidepoint(mx, my):
                            game.ach_games_index = index - 1
                        elif index < games_count - 1 and games_next.collidepoint(mx, my):
                            game.ach_games_index = index + 1                    
        ...
        case "ADVANCED_SETTINGS":
            ...
            if buttons["toggle_terminal_sound"].collidepoint(mx, my):
                game.settings["terminal_sound_enabled"] = not game.settings["terminal_sound_enabled"]
                game.persist_settings()
                return
            
            if buttons["language_dropdown"].collidepoint(mx, my):
            ...
            if getattr(game, "language_dropdown_open", False):
                for i, (label, internal) in enumerate(helpers.available_languages()):
                    rect = buttons.get(f"language_option_{i}")
            ...
            for action in con.DEFAULT_KEYBINDINGS:
                rect = buttons[f"keybind_{action}"]
                if rect.collidepoint(mx, my):
        ...
        case "HISTORY": # If the player is in the history menu
            ...
            filter_buttons = helpers.history_filter_buttons(game) # Load the filter buttons
        ...
        case "DELETE_HISTORY": # The delete screen
            buttons = helpers.draw_delete_confirm(game, mx, my) # Get the buttons
        ...
        case "HISTORY_DETAIL": # If in a history file
            if helpers.BTN_BACK.collidepoint(mx, my): # If clicked on back
                game.open_history() # Reset the scrol etc
                return # Task is over
            elif helpers.DETAIL_EXPORT_BUTTON.collidepoint(mx, my): # Export as mp4 wanted
                game.export_selected_to_mp4() # Render the match into a video
                return # Task is over
            elif helpers.DETAIL_QUALITY_BUTTON.collidepoint(mx, my): # Change the quality
                game.cycle_export_quality() # Change the quality
                return # task is over
            elif helpers.DETAIL_FPS_BUTTON.collidepoint(mx, my): # change the frame rate
                game.cycle_export_fps() # Change the frame rate
            elif helpers.DETAIL_RESET_BUTTON.collidepoint(mx, my): # End view wanted
        ...
        case "HANNAH": # If the player is in the first easter egg
            if game.hannah_open_index is None: # The scrollable overview
                buttons = helpers.hannah_buttons() # Load the buttons
                if buttons["back"].collidepoint(mx, my): # If the player clicked back
        ...
        case "PLAY": # If the player is playing
            buttons = helpers.play_buttons() #  load buttons
            button_actions_game = {
            ...
            for name, action in button_actions.items():
                if buttons[name].collidepoint(mx, my):
                    action()
                    return
