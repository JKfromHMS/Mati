    ...
    while True: # Until the programm gots quit
        ...
        for event in pg.event.get(): # Everything happend got saved
            if event.type == pg.QUIT: # If the user want to quit
                ...
                if event.button == 1: # Only left clicks
                    if game.state == "ADVANCED_SETTINGS":
                        for which in ("game_volume", "terminal_volume"):
                            track = s.advanced_settings_layout(game).get(which)
                            if track and track.collidepoint(mx, my):
                                ...
                                if which == "game_volume":
                                    ...
                                    game.settings["terminnal_volume"] = value
                                started_drag = True
                                break
                    if not started_drag:
                ...
                if game.state in ("HISTORY", "HISTORY_DETAIL"): # If the player tries to scroll in history or detail view
                    ...
                    _, time_tile = s.achievements_tile_rects()
        ...
        if getattr(game, "slider_drag", None):
            if pg.mouse.get_pressed()[0]:
                track = s.advanced_settings_layout(game).get(game.slider_drag)
                if track:
                    value = w.value_for_slider_x(track, mx)
...
def handle_click(game, click_time, mx, my, button): # Let the programm work with a click
    ...
    match state: # py 3.10 method to make if else more efficient
        ...
        case "ACHIEVEMENTS":
            if con.BTN_BACK.collidepoint(mx, my):
                ...
                if game.achievements_page == 0:
                    games_tile, _ = s.achievements_tile_rects()
                    if games_tile.collidepoint(mx, my):
                        games_prev, games_next = s.achievements_games_nav_rects()
                        games_count = len(con.GENERAL_GAME_MILESTONES)
                        index = getattr(game, "ach_games_index", 0)
        ...
        case "ADVANCED_SETTINGS":
            buttons = s.advanced_settings_layout(game)
            ...
            for i, option in enumerate(con.INPUT_ORDER_OPTIONS):
                ...
                if rect.collidepoint(mx, my):
                    game.settings["input_order"] = option
                    game.persist_settings()
                    return
        ...
        case "HISTORY_DETAIL": # If in a history file
            if con.BTN_BACK.collidepoint(mx, my): # If clicked on back
                ...
                return # Task is over
            elif s.detail_export_button().collidepoint(mx, my): # Export as mp4 wanted
                game.export_selected_to_mp4() # Render the match into a video
                return # Task is over
            elif s.detail_quality_buttons().collidepoint(mx, my): # Change the quality
                game.cycle_export_quality() # Change the quality
                return # task is over
            elif s.detail_fps_buttons().collidepoint(mx, my): # change the frame rate
                game.cycle_export_fps() # Change the frame rate
            elif s.detail_reset_button().collidepoint(mx, my): # End view wanted
