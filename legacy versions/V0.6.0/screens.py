...
def focus_order(game): # Gives the orderd, navigable elements in the current screen
    ...
    match state:
        ...
        case "ADVANCED_SETTINGS":
            order = list(advanced_settings_layout(game).items())
...
def _hardcoded_neighbor(game, key, direction):
    ...
    match state:
        ...
        case "ADVANCED_SETTINGS":
            ...
            left_chain = ["back", "game_volume", "terminal_volume", "toggle_terminal_sound", "language_dropdown"]
            if dropdown_open:
                left_chain += [f"language_option_{i}" for i in range(len(lang.available_languages()))]
                
            right_chain = [f"input_order_{i}" for i in range(len(con.INPUT_ORDER_OPTIONS))]
            right_chain += [f"keybind_{action}" for action in con.DEFAULT_KEYBINDINGS]
            
            def _chain_step(chain, k, d):
                if k not in chain:
                    return None
                idx = chain.index(k)
                if d == "up":
            ...
            if key in left_chain:
                if direction in {"up", "down"}:
                    return _chain_step(right_chain, key, direction)
                # if direction == "left" and left_chain:
                #    return left_chain[min(right_chain.index(key), len(left_chain) - 1)]
                return None
...
def move_focus(game, direction): # How to move the focus
    ...
    if state == "SETTINGS":
        ...
        if key == left_terminus:
            ...
            if nxt == left_terminus and key != left_terminus:
        ...
        if key == "toggle_live_clock":
            ...
            if nxt == "toggle_live_clock" and key != "toggle_live_clock":
...
def _settings_sections(game):
    ...
    if terminal_found:
        ...
        sections.extend([
            (lang.t("gameplay", "Gameplay"), gameplay_rows, left_x, y),
            (lang.t("timer", "Timer"), timer_rows, left_x, y + 138),
            (lang.t("ultra_mode", "Ultra Mode"), ultra_rows, right_x, y),
            (lang.t("display", "Display"), display_rows, right_x, y + 138)
        ])

    else:
        sections.extend([
            (lang.t("gameplay", "Gameplay"), gameplay_rows, mid_x, y),
            (lang.t("timer", "Timer"), timer_rows, left_x, y + 138),
            (lang.t("display", "Display"), display_rows, right_x, y + 138)
        ])
...
def advanced_settings_layout(game):
    ...
    if getattr(game, "language_dropdown_open", False):
        for i, (label, _internal) in enumerate(lang.available_languages()):
...
def draw_advanced_settings(game, mx, my):
    ...
    buttons = advanced_settings_layout(game)
    
    w.draw_title(lang.t("advanced_settings_title", "Advanced Settings"), con.TEXT_COLOR, con.WIDTH // 2, 55)
    w.draw_button(con.BTN_BACK, lang.t("back", "Back"), con.BTN_BACK.collidepoint(mx, my), focused=(fk == "back"))
    
    # --- Volume ---
    w.draw_small(lang.t("volume", "Volume"), con.TEXT_COLOR, 60, 105, center=False)
    w.draw_slider(buttons["game_volume"], game.settings.get("game_volume", 1.0), buttons["game_volume"].collidepoint(mx, my), label=lang.t("game_volume", "Game volume"))
    w.draw_slider(buttons["terminal_volume"], game.settings.get("terminal_volume", 1.0), buttons["terminal_volume"].collidepoint(mx, my), label=lang.t("terminal_volume", "Terminal Volume"))
    ...
    current_display = next((label for label, internal in lang.available_languages() if internal == lang.current_language()), "English")
    ...
    if getattr(game, "language_dropdown_open", False):
        for i, (label, internal) in enumerate(lang.available_languages()):
    ...
    current_order = game.settings.get("input_order", "column_row_action")
    for i, option in enumerate(con.INPUT_ORDER_OPTIONS):
        key = f"input_order_{i}"
        rect = buttons[key]
        active = option == current_order
...
def draw_about(game, mx, my):
    ...
    w.draw_button(con.BTN_BACK, lang.t("back", "Back"), con.BTN_BACK.collidepoint(mx, my), focused=(fk == "back")) # Let back button draw
...
def draw_stats(game, mx, my): # Draws the graphical version of the stats
    ...
    fk = _effective_focus_key(game, mx, my) # Get the keyboard focus
    w.draw_button(con.BTN_BACK, lang.t("back", "Back"), con.BTN_BACK.collidepoint(mx, my), focused=(fk == "back")) # Draw back button with focus option
...
_ACH_TILE_BTN_SIZE = 34
ACH_GAMES_PREV_RECT = pg.Rect(
    ACH_GAMES_TILE_RECT.x + 8, ACH_GAMES_TILE_RECT.y + 43, _ACH_TILE_BTN_SIZE, _ACH_TILE_BTN_SIZE,
)
ACH_GAMES_NEXT_RECT = pg.Rect(
    ACH_GAMES_TILE_RECT.right - 8 - _ACH_TILE_BTN_SIZE, ACH_GAMES_TILE_RECT.y + 43, _ACH_TILE_BTN_SIZE, _ACH_TILE_BTN_SIZE,
)

def achievement_nav_rects():
    return ACH_PREV_RECT, ACH_NEXT_RECT

def achievements_games_nav_rects():
    return ACH_GAMES_PREV_RECT, ACH_GAMES_NEXT_RECT

def achievements_tile_rects():
    return ACH_GAMES_TILE_RECT, ACH_TIME_TILE_RECT

def _fmt_total_time(ms):
    total_seconds = ms // 1000
    hours = total_seconds // 3600
    minutes = (total_seconds % 3600) // 60
    return f"{hours}h {minutes:02}min" if hours else f"{minutes}min"

def _milestone_default_index(keys, achievements):
    for i, key in enumerate(keys):
        if not achievements.get(key):
            return i
    return len(keys) - 1

def _milestone_progress(values, current):
    for i, target in enumerate(values):
        if current < target:
            prev_value = values[i - 1] if i > 0 else 0
            span = target - prev_value
            frac = (current - prev_value) / span if span else 1.0
            return max(0.0, min(1.0, frac)), prev_value, target, False
    last = values[-1] if values else 0
    return 1.0, last, last, True

def _milestone_fraction(current, target):
    if target <= 0: 
        return 1.0
...
def _draw_milestone_tile(rect, title, keys, values, achievements, current_value, index, fmt_value, mx, my, nav_rects = None):
    ...
    if nav_rects and hovering:
        prev_rect, next_rect = nav_rects
        if index > 0:
            w.draw_button(prev_rect, "<", prev_rect.collidepoint(mx, my))
        if index < len(keys) - 1:
            w.draw_button(next_rect, ">", next_rect.collidepoint(mx, my))
            
    remaining = max(0, target - current_value)
    frac = _milestone_fraction(current_value, target)
    bar_rect = pg.Rect(rect.x + 16, rect.y + 226, rect.width - 32, 18)
    
    if remaining <= 0:
        w.draw_small(lang.t("milestones_reached", "Reached!"), con.GOLD, rect.centerx, rect.y + 170)
    else:
...
def draw_achievements(game, mx, my):
    ...
    if is_general_page:
        ...
        if games_index is None:
            games_index = _milestone_default_index(games_keys, achievements)
        games_nav = achievements_games_nav_rects()
        game.ach_games_index = _draw_milestone_tile(
            ACH_GAMES_TILE_RECT, lang.t("games_played", "Games Played"),
        ...
        if time_index is None:
            time_index = _milestone_default_index(time_keys, achievements)
        game.ach_time_index = _draw_milestone_tile(
            ACH_TIME_TILE_RECT, lang.t("total_playtime", "Total Play Time"),
            time_keys, time_values_ms, achievements, total_times_ms,
            time_index, _fmt_total_time, mx, my, nav_rects= None,
        )
    else:    
        y = 178
        row_h = 28
...
def draw_history(game, mx, my): # Draw the history
    ...
    fk = _effective_focus_key(game, mx, my) # Get the keyboard focus
    w.draw_button(con.BTN_BACK, lang.t("menu", "Menu"), con.BTN_BACK.collidepoint(mx, my), focused=(fk == "back")) # Draw the back button
...
def draw_history_detail(game, mx, my): # the draw the detailed history view
    ...
    fk = _effective_focus_key(game, mx, my) # Get the keyboard focus
    w.draw_button(con.BTN_BACK, lang.t("back", "Back"), con.BTN_BACK.collidepoint(mx, my), focused=(fk == "back")) # The Back button
    ...
    # n = len(data["grid"])
    #if n == 14:
     #   if minutes < 1: w.draw_tiny(lang.t("time", "Time: ") + f"{seconds:02}:{ms:03}s", con.TEXT_COLOR, offset_x, footer_y - 10, center=False) # Time under the grid
      #  if minutes >= 1: w.draw_tiny(lang.t("time", "Time: ") + f"{minutes}:{(seconds%60):02}:{ms:03}min", con.TEXT_COLOR, offset_x, footer_y - 10, center=False)
       # w.draw_tiny(lang.t("hints", "Hints used: ") + f"{hints_used}", con.TEXT_COLOR, offset_x + 220, footer_y - 10, center=False) # Hints used under the time
        #if is_start_view: # The start of the game
         #   w.draw_tiny(lang.t("action_start", "Action: Start"), con.TEXT_COLOR, offset_x, footer_y + 11, center=False) # Describe it as start
         ...
         #   label = con.ACTION_LABELS.get(last_action.get("type", "Left"), last_action.get("type", "?")) # Get the right action name
          #  w.draw_tiny(lang.t("action", "Action: ") + str(label) + lang.t("row", " (Row: ") + str(last_action['r'] + 1) + lang.t("column", " / Column: ") + str(last_action['c'] + 1) + ")", con.TEXT_COLOR, offset_x, footer_y + 11, center=False)
        #else: # f"Action: {label} (Row: {last_action['r'] + 1} Column: {last_action['c'] + 1})"
         #   w.draw_tiny(lang.t("end", "End of game"), con.DIMMED_TEXT_COLOR, offset_x, footer_y + 11, center=False) # Or that it is the last
...
def draw_hannah(game, mx, my): # Creates first easter egg view
    ...
    if game.hannah_open_index is None:
        ...
        w.draw_button(con.BTN_BACK, lang.t("menu", "Menu"), con.BTN_BACK.collidepoint(mx, my), focused=(fk == "back"))
        screen = w.get_screen()
