...
import lang              # To translate between languages
    ...
    while True: # Until the programm gots quit
        ...
        for event in pg.event.get(): # Everything happend got saved
            if event.type == pg.QUIT: # If the user want to quit
                ...
                if active_terminal and active_terminal.ultra_game: # If the player is playing ultra
                ...
                if getattr(game, "rebind_listening", None):
                    # We need to teach the programm that not every key should trigger this.
                    MODIFIER_KEYS = {
                        pg.K_LCTRL, pg.K_RCTRL,
                        pg.K_LMETA, pg.K_RMETA,
                        pg.K_LALT, pg.K_RALT,
                ...
                if game.matches_binding("fullscreen", event): # If f11 pressed
                    ...
                    w.set_real_screen(real_screen) # Keep the programm running
                elif game.matches_binding("pause", event) and game.state == "PLAY": # If the player pressed p in the game
                    game.toggle_pause() # Make the game paused or continued
                elif game.matches_binding("new_round", event) and game.state == "PLAY": # Start a new round with Ctrl/Cmd+N
                    game.restart_same() # Same as the New button
                elif game.matches_binding("menu", event) and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE", "HANNAH", "PLAY", "TERMINAL", "STATS", "ACHIEVEMENTS", "ADVANCED_SETTINGS"):
                    if game.state == "PLAY":
                ...
                elif game.matches_binding("undo", event) and game.state == "PLAY": # If the player pressed CTRL and z and is playing
                    ...
                    elif game.matches_binding("right_click", event): game.click_cell(game.cursor_r, game.cursor_c, right_click=True) # Make a right click
                    elif event.key == pg.K_u: game.undo() # Undo the last action
                    elif game.matches_binding("hint", event): game.use_hint() # Use an hint
                    ...
                    game.return_focus["SETTINGS"] = game.focus_key # Remember where to come back to
                ...
                elif game.alt_control and event.key in (pg.K_UP, pg.K_DOWN, pg.K_LEFT, pg.K_RIGHT) and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE", "HANNAH", "STATS", "ADVANCED_SETTINGS"): # Keyboard menu navigation
                    was_following_mouse = ah.effective_focus_key(game, mx, my) != game.focus_key # Was the mouse-hover shown instead of the real focus
                    ...
                    if not was_following_mouse: # Only actually move if the real focus was already the one shown
                        ...
                        ah.move_focus(game, direction) # Move the focus
                elif game.alt_control and event.key in (pg.K_SPACE, pg.K_RETURN) and game.state in ("MENU", "SETTINGS", "ABOUT", "HISTORY", "HISTORY_DETAIL", "DELETE_HISTORY", "RESUME_CHOICE", "HANNAH", "STATS", "ACHIEVEMENTS", "ADVANCED_SETTINGS"): # Keyboard click replacement
                    key = ah.effective_focus_key(game, mx, my) # Get the real focused key
                    lookup = dict(ah.focus_order(game)) # Key -> rect
                    ...
                    min_scroll = helpers.hannah_scroll_bounds() # Do not scroll behind the end
                ...
                if game.state in ("HISTORY", "HISTORY_DETAIL"): # If the player tries to scroll in history or detail view
                    ...
                    time_tile = helpers.ACH_TIME_TILE_RECT
        ...
        if game.alt_control and (game.state in ("HISTORY", "HISTORY_DETAIL") or in_hannah_strip or in_slider_dot_mode): # If repeat should be active
...
def handle_click(game, click_time, mx, my, button): # Let the programm work with a click
    ...
    match state: # py 3.10 method to make if else more efficient
        case "MENU": # If the player is in the menu
            buttons = bt.menu_buttons.get(game) # Load the buttons
            for n in con.DIFFICULTIES: # For every grid size
                if buttons[f"start_{n}"].is_clicked(mx, my): # If a grid size button clicked
                    game.return_focus["MENU"] = f"start_{n}"
                    game.new_game(n) # Start a new game with the given grid size
                    return # End of this task
            if buttons["settings"].is_clicked(mx, my): # If the player clicked on settings
                game.return_focus["MENU"] = "settings"
                game.state = "SETTINGS" # Set state to settings
            elif buttons["history"].is_clicked(mx, my): # If the player clicked on history
                game.return_focus["MENU"] = "history"
                game.open_history() # Open the history
            elif buttons["quit"].is_clicked(mx, my): # If player want to quit
                pg.quit() # End the game
                sys.exit() # Close the game
    
        case "SETTINGS": # If the player is in the settings
            buttons = bt.settings_buttons.get(game) # Load buttons
            if game.timer_enabled:
                if buttons["toggle_ms"].is_clicked(mx, my): # If the player clicked on ms
                    game.timer_ms = not game.timer_ms # toggle between on and off
            if buttons["back"].is_clicked(mx, my): # If the player clicked back
                game.state = "MENU" # Return to menu
            elif buttons["toggle_history"].is_clicked(mx, my): # If the player clicked history save
                game.save_history = not game.save_history # Toggle if save or not
            elif buttons["toggle_timer"].is_clicked(mx, my): # If the player clicked the timer toggle
            ...
            elif buttons["toggle_sound"].is_clicked(mx, my): # If the player clicked on the sound
                game.sounds.enabled = not game.sounds.enabled # Toggle this state
            elif buttons["toggle_fullscreen"].is_clicked(mx, my): # If the player clicked on fullscreen
                game.request_fullscreen_toggle = True # activate Fullscreen or deactivate
            elif buttons["toggle_alt_control"].is_clicked(mx, my): # If the Player want to toggle keyboard navigation
            ...
            elif buttons["toggle_live_clock"].is_clicked(mx, my): # If the player clicked on the live clock
            ...
            elif "toggle_ultra_timer" in buttons and buttons["toggle_ultra_timer"].is_clicked(mx, my): # If the player clicked on the ultra timer
            ...
            elif "toggle_ultra_timer_ms" in buttons and buttons["toggle_ultra_timer_ms"].is_clicked(mx, my): # If the player clicked on ultra timer ms
                game.ultra_timer_ms = not game.ultra_timer_ms # Toggle on and off
            elif "toggle_ultra_timer_clock" in buttons and buttons["toggle_ultra_timer_clock"].is_clicked(mx, my): # If the player clicked on ultra timer clock
                game.ultra_timer_show_clock = not game.ultra_timer_show_clock # Toggle on and off
            game.persist_settings() # Save the new settings
            
            if buttons["stats"].is_clicked(mx, my): # If the player clicked on stats
            ...
            elif buttons["about"].is_clicked(mx, my): # If the player clicked on about
                game.return_focus["SETTINGS"] = "about"
                game.state = "ABOUT" # go to about
            #elif buttons["advanced"].is_clicked(mx, my):
            #    game.return_focus["SETTINGS"] = "advanced"
            #    game.language_dropdown_open = False
            #    game.state = "ADVANCED_SETTINGS"
            
        case "ABOUT": # If the player is in the about
            if bt.about_buttons.get(game)["back"].is_clicked(mx, my): # If the player clicked back
                game.state = "SETTINGS" # Return to settings
                
        case "STATS": # If the player is in the stats
            if bt.stats_buttons.get(game)["back"].is_clicked(mx, my): # If the player clicked on back
                game.state = "SETTINGS" # Return to settings
        ...
        case "ACHIEVEMENTS":
            ...
            if nav_buttons["back"].is_clicked(mx, my):
                ...
                if game.achievements_page == 0:
                    games_tile = helpers.ACH_GAMES_TILE_RECT
        ...
        case "HISTORY": # If the player is in the history menu
            if bt.history_back_button.get(game)["back"].is_clicked(mx, my): # If the player clicked on back
                game.state = "MENU" # return to the menu
                return # Task is over
        
            filter_buttons = bt.history_filter_buttons.get(game) # Load the filter buttons
            if filter_buttons["size_all"].is_clicked(mx, my): # Clicked on all sizes
                game.toggle_size_filter(None) # This filter is the main, or no
                return # Task is over
            for n in con.DIFFICULTIES: # For each difficultie
                if filter_buttons[f"size_{n}"].is_clicked(mx, my): # If a size is clicked
                    game.toggle_size_filter(n) # Make the filter work
                    return # Task is over
            if filter_buttons["top10"].is_clicked(mx, my): # For the top 10 filter
                game.toggle_top10_filter() # Make the filter work
                return # Task is over
            if "ultra" in filter_buttons and filter_buttons["ultra"].is_clicked(mx, my): # For the ultra filter
            ...
            for i, entry in enumerate(entries): # For each file in history files
                delete_rect = helpers.history_delete_rect(i, game.history_scroll_y) # Get positions for delete buttons
                entry_rect = helpers.history_entry_rect(i, game.history_scroll_y) # Get the positions of the buttons
                if delete_rect.collidepoint(mx, my): # If the player want to delete something
                    ...
                    return # Task is over
                elif entry_rect.collidepoint(mx, my): # If the player clicked on a file
        ...
        case "DELETE_HISTORY": # The delete screen
            buttons = bt.delete_confirm_buttons.get(game) # Get the buttons
        
            if buttons["yes"].is_clicked(mx, my): # If yes clicked
                ...
                return # task is done
            elif buttons["no"].is_clicked(mx, my): # If clicked no
                game.file_to_delete = None # clear the file save
                game.state = "HISTORY" # Return to History
                return # task is done
        ...
        case "HISTORY_DETAIL": # If in a history file
            ...
            if detail_buttons["back"].is_clicked(mx, my): # If clicked on back
                game.open_history() # Reset the scrol etc
                return # Task is over
            elif detail_buttons["export_mp4"].is_clicked(mx, my): # Export as mp4 wanted
                game.export_selected_to_mp4() # Render the match into a video
                return # Task is over
            elif detail_buttons["export_quality"].is_clicked(mx, my): # Change the quality
                game.cycle_export_quality() # Change the quality
                return # task is over
            elif detail_buttons["export_fps"].is_clicked(mx, my): # change the frame rate
                game.cycle_export_fps() # Change the frame rate
            elif detail_buttons["detail_reset"].is_clicked(mx, my): # End view wanted
                game.reset_detail_view() # Let the game reset to the end view
                return # Task is over
            elif game.selected_history_data: # Clicked on a action
                display_actions = helpers.detail_display_actions(game.selected_history_data) # Load the action
                for i in range(len(display_actions)): # Froe each action
                    rect = helpers.detail_action_rect(i, game.detail_scroll_y) # Scroll area
                    ...
                    if rect.collidepoint(mx, my): # If the player clicked it
                        game.select_detail_action(i - 1) # Show action
                        return # Task is done
                
        case "RESUME_CHOICE": # If the player is asked if he want to continue
            buttons = bt.resume_choice_buttons.get(game) # Load the buttons
            if buttons["resume"].is_clicked(mx, my): # If the player want to resume
                game.resume_paused(game.pending_new_n, game.pending_new_ultra) # countine the paused match
                return # task is done
            elif buttons["new"].is_clicked(mx, my): # If the player ant create a new
                game.discard_and_start(game.pending_new_n, game.pending_new_ultra) # throw the paused away
                return # task is done
            elif buttons["cancel"].is_clicked(mx, my): # If the player want to cancle
        ...
        case "HANNAH": # If the player is in the first easter egg
            if game.hannah_open_index is None: # The scrollable overview
                buttons = bt.hannah_buttons.get(game) # Load the buttons
                if buttons["back"].is_clicked(mx, my): # If the player clicked back
                ...
                for i, lvl in enumerate(game.hannah_levels): # Check every tile
                    if lvl is None: # The spaces
                        continue # next slot
                    rect = helpers.hannah_tile_rect(i, game.hannah_scroll_x) # its position
                ...
                if buttons["back"].is_clicked(mx, my): # If the player clicked back
                    ...
                    return # task is done
                if buttons["undo"].is_clicked(mx, my): # Clicked on undo
        ...
        case "PLAY": # If the player is playing
            buttons = bt.play_buttons.get(game) #  load buttons
            button_actions_game = {
            ...
            if not game.paused:
                button_actions = button_actions_game
            else:
                button_actions = button_actions_pause
            ...
            offset_x, offset_y = helpers.play_grid_offset(game.n) # Load the offsets
...
def _scrollbar_target(game): # Which scrollbar (if any) is relevant
    match game.state:
        case "HISTORY": # The history list
            entries = game.filtered_history() # The currently visible entries
            track = helpers.history_scrollbar_track() # its track
            visible = con.HISTORY_VISIBLE_BOTTOM - con.LIST_TOP # The visible height
            content = helpers.history_content_height(len(entries)) # The full content height
            return ("history", track, content, visible, True) # Vertical bar
        
        case "HISTORY_DETAIL": 
            if game.selected_history_data: # The action list
                display_actions = helpers.detail_display_actions(game.selected_history_data) # All entries
                track = helpers.DETAIL_SCROLLBAR_TRACK # its track
                return ("detail", track, len(display_actions) * 34, 440, True) # Vertical bar
        
        case "HANNAH": 
            if getattr(game, "hannah_open_index", None) is None: # The tile view
                track = helpers.hannah_scrollbar_track() # its track
                return ("hannah", track, helpers.hannah_content_width(), con.HANNAH_VISIBLE_LENGTH, False) # Horizontal bar
...
def _scrollbar_current_offset(game, which): # The current scrollbar offset
    match which:
        case "history": 
            return -game.history_scroll_y
    
        case "detail": 
            return -game.detail_scroll_y
    
        case "hannah": 
            return -game.hannah_scroll_x
...
def handle_scroll(game, event): # Make scrolling possible
    ...
    match game.state: # py 3.10 method to make if else more efficient
        case "HISTORY": # If scroll in history
            game.history_scroll_y += delta # Scroll in the same direction as the scrollbar drag and keyboard focus
            count = len(game.filtered_history()) # Show right history
            max_scroll = 0 # Not over the top
            min_scroll = helpers.history_scroll_bounds(count) # Not under the bottom
        ...
        case "HISTORY_DETAIL": # If scroll in history detail
            game.detail_scroll_y += delta # Scroll in the same direction as the scrollbar drag and keyboard focus
            count = len(helpers.detail_display_actions(game.selected_history_data)) if game.selected_history_data else 0 # Show right history
            max_scroll = 0 # Not over the top
        ...
        case "HANNAH": # In scrollable view
            ...
            game.hannah_scroll_x += delta # scroll in the same direction as the scrollbar drag and keyboard focus
            min_scroll = helpers.hannah_scroll_bounds() # Do not scrolll to further
...
def draw(game, mx, my): # Let the programm draw the views
    match game.state: # py 3.10 method to make if else more efficient
        ...
        case "ADVANCED_SETTINGS":
            s.draw_advanced_settings(game, mx, my)
