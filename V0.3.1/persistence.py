...
def load_settings_and_stats(): # Load settings, stats and paused games
    if not os.path.exists(SF): # If the file does not exsist
        return dict(DEFAULT_SETTINGS), _default_stats(), {} # Give the defaults back
    try: # try to read it
        with open(SF, "r") as f: # Open the file
            data = json.load(f) # Load the content
    except (OSError, json.JSONDecodeError): # If the file is broken or the system denies it
        return dict(DEFAULT_SETTINGS), _default_stats(), {} # Give back the default
    ...
    return settings, stats, paused, hannah # Give everything back

def save_settings_and_stats(settings, stats, paused, hannah=None): # Save the settings, stats and paused games in a file
    if hannah is None: # If the player has not found the easter egg
        hannah = [] # Fallback, empty value
        if os.path.exists(SF): # If there is the setting file
            try: # Try to load existing Hannah progress
                with open(SF, "r") as f: # Open the settings
                    old = json.load(f) # Load the content
                hannah = old.get("hannah", []) # Load the hannah value if available
            except (OSError, json.JSONDecodeError): # If it is broken
                hannah = [] # Start empty
    data = {"settings": settings, "stats": stats, "paused": paused, "hannah": hannah} # Bundle everthing 
    with open(SF, "w") as f: # Open/create the file
        json.dump(data, f) # Write it
