...
from config import XOR_KEY               # The en- and decode key
from config import DEFAULT_SETTINGS      # The default settings
from config import DEFAULT_ACHIEVEMENTS  # The default achievements
...
def _xor(raw): # XOR every byte to make it uneditable
    key = XOR_KEY # Short name, introduction because it should be in config later
...
def load_settings_and_stats(): # Load settings, stats and paused games
    ...
    if not os.path.exists(path): # If the file does not exsist
        return dict(DEFAULT_SETTINGS), _default_stats(), {}, [], dict(DEFAULT_ACHIEVEMENTS) # Give the defaults back
    try: # try to read it
        ...
        return dict(DEFAULT_SETTINGS), _default_stats(), {}, [], dict(DEFAULT_ACHIEVEMENTS) # Give back the default
    ...
    return settings, stats, paused, hannah, achievements # Give everything back

def save_settings_and_stats(settings, stats, paused, hannah=None, achievements=None): # Save the settings, stats and paused games in a file
    ...
    if hannah is None or achievements is None: # If the player has not found the easter egg
        old_hannah, old_achievements = [], dict(DEFAULT_ACHIEVEMENTS) # Fallback, empty value
        ...
        if achievements is None: achievements = old_achievements
        
    data = {"settings": settings, "stats": stats, "paused": paused, "hannah": hannah, "achievements": achievements} # Bundle everthing 
