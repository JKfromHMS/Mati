...
def settings_buttons(game): # Create the buttons for the settings
    if game.timer_enabled:
        return{
            ...
            "about": pg.Rect(con.WIDTH // 2 - 110, 450, 220, 42)
        } # The buttons if all are visible
    else:
        return{
            "back": con.BTN_BACK,
            "toggle_history": pg.Rect(con.WIDTH // 2 - 110, 150, 220, 42),
            "toggle_timer": pg.Rect(con.WIDTH // 2 - 110, 200, 220, 42),
            "toggle_sound": pg.Rect(con.WIDTH // 2 - 110, 250, 220, 42),
            "toggle_fullscreen": pg.Rect(con.WIDTH //  2 - 110, 300, 220, 42),
            "about": pg.Rect(con.WIDTH // 2 - 110, 450, 220, 42)
        } # The buttons if some are visible
...
def draw_about(mx, my):
    ...
    w.draw_button(con.BTN_BACK, "Back", con.BTN_BACK.collidepoint(mx, my)) # Let back button draw
...
def draw_delete_confirm(game, mx, my): # Drawn the menu to delete a file
    ...
    w.draw_button(btn_yes, "Delete", btn_yes.collidepoint(mx, my)) # Draw yes button
    w.draw_button(btn_no, "Cancel", btn_no.collidepoint(mx, my)) # Draw the no button
...
def draw_history(game, mx, my): # Draw the history
    ...
    w.draw_button(con.BTN_BACK, "Menu", con.BTN_BACK.collidepoint(mx, my)) # Draw the back button
    ...
    if "ultra" in filter_buttons:
        ...
        w.draw_toggle_button(ultra_rect, "Ultra", ultra_rect.collidepoint(mx, my), game.history_filter_ultra) # Draw the ultra button
...
def draw_history_detail(game, mx, my): # the draw the detailed history view
    ...
    w.draw_button(con.BTN_BACK, "Back", con.BTN_BACK.collidepoint(mx, my)) # The Back button
...
def draw_play(game, mx, my): # draw the play view
    ...
    for r in range(n): # For every row
        ...
        if game.paused: # If the game is paused
            ...
            w.draw_button(buttons["menu"], "Menu", buttons["menu"].collidepoint(mx, my)) # Let the back button be drawn
            w.draw_button(buttons["break"], "Continue", buttons["break"].collidepoint(mx, my), enabled=not game.won) # The return to game button
            w.draw_button(buttons["new"], "New", buttons["new"].collidepoint(mx, my)) # Show the restart button
